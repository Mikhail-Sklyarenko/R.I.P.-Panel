...
class NavController:
        ...
        *,
        ...
        if self._state == NavState.STUCK_ESCAPE:
            if now < self._escape_until:
                ...
                # Product: thrust forward while turning — never orbit in place.
            ...
            result = self._make_result(
                ...
                pose=pose,
                dist=dist_goal,
                forward_held=now < self._escape_until,
            )
            self._last_pose = pose
            return result
        if self._phase == NavPhase.ENTRY:
    ...
    def _drive_toward_target(
        ...
        *,
        ...
        stalled_for = max(0.0, now - self._last_progress_at) if self._last_progress_at else 0.0
        fail_after = self._pack.humanize.forward_fail_open_after_sec
        force_crawl = stalled_for >= fail_after
        force_walk = stalled_for >= fail_after * 2.0
        if force_crawl and now - self._last_fail_open_log_at >= 5.0:
            self._log.info(
                "nav: forward fail-open stall=%.1fs crawl=%s walk=%s",
                stalled_for,
                force_crawl,
                force_walk,
            )
        ...
        if motion.paused:
            ...
            return self._make_result(
                ...
                forward_fail_open=force_crawl or force_walk,
            )
