...
**Autobuy rifle (DM):** `cl_dm_buyrandomweapons 0` + **F5**→`buy_rifle_dm` in `fsm.cfg` (Insert bind kept for manual). csgobot жмёт **F5** каждые 1 s + staggered respawn @ 0,4/0,9/1,4 s. Env: `CSGOBOT_AUTO_BUY`, `CSGOBOT_AUTO_BUY_INTERVAL`, `CSGOBOT_AUTOBUY_RESPAWN_DELAYS_MS`, `CSGOBOT_AUTOBUY_RESPAWN_COOLDOWN_MS`. См. `docs/DM_CFG.md`.
