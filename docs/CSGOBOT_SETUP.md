...
**Autobuy rifle (DM):** `cl_dm_buyrandomweapons 0` + Insert→`buy_rifle_dm` in `fsm.cfg`. csgobot: burst every **1 s** + **staggered respawn** Insert @ 0,4/0,9/1,4 s. Env: `CSGOBOT_AUTO_BUY`, `CSGOBOT_AUTO_BUY_INTERVAL`, `CSGOBOT_AUTOBUY_RESPAWN_DELAYS_MS`, `CSGOBOT_AUTOBUY_RESPAWN_COOLDOWN_MS`. См. `docs/DM_CFG.md`.
