...
| Spin in place / stand still / rare 1–2 steps | Pull place-nav: `mode=world` + `path=…` + `place localizer loaded`; not seed/DR; stuck = no radar flow |
| Wait forever without walking | Need CS2 location label under radar; log `wait_place` until label matches `hud_ref` |
