...
| Wait forever without walking | Need CS2 location label; log `wait_place` / `place=…`; `CSGOBOT_NAV_NO_MACRO` is default — expect path seek not generic_dm walls |
| Spin in place / stand still / rare 1–2 steps | Pull place-nav: `mode=world` + `path=…` + `place=… score=`; abort-macro on world; hold=4s |
| Bot walks into walls / random turns | Macro was cosplaying nav — default macro **off**; seek uses waypoint path only |
